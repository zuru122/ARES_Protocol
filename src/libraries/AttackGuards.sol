...
library AttackGuards {
    ...
    function enforceDailyLimit(RateLimit storage _self, uint256 _amount) internal {
    ...
    function isWithinDailyLimit(RateLimit storage _self, uint256 _amount) internal view returns (bool) {
    ...
    function recordSnapshot(Snapshot storage _self, bytes32 _proposalId) internal {
        _self.proposalBlock[_proposalId] = block.number;
    }

    function recordBalance(Snapshot storage _self, address _user, uint256 _balance) internal {
        _self.balanceAt[_user][block.number] = _balance;
    }

    function getVotingPower(Snapshot storage _self, address _user, bytes32 _proposalId)
        internal
        view
        returns (uint256)
    {
